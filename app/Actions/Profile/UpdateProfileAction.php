...
class UpdateProfileAction
{
    ...
    {
        // Validate the data
        $validated = $this->validate($profile, $data);

        return DB::transaction(function () use ($profile, $validated, $data) {
            ...
            // Sync reviewer journals if provided
            if ($reviewerJournals !== null && is_array($reviewerJournals)) {
                $profile->reviewerJournals()->sync($reviewerJournals);
            }
     ...
     * @throws ValidationException
     */
    protected function validate(Profile $profile, array $data): array
    {
        $validator = Validator::make($data, [
            'profile_type' => ['sometimes', Rule::in(['academic', 'institutional', 'organizational', 'personal'])],
