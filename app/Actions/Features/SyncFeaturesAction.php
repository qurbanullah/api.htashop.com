...
class SyncFeaturesAction
{
    public function execute(Model $featurable, array|Collection $features, string $type = 'product'): Collection
    {
        ...
        // Get old feature IDs before sync
        $oldFeatureIds = $featurable->features->pluck('id')->toArray();
        ...
        // Sync features
        $featurable->features()->sync($syncData);

        // Get new feature IDs after sync
        $newFeatureIds = $featureModels->pluck('id')->toArray();

        // Decrement usage for removed features
        $removedFeatureIds = array_diff($oldFeatureIds, $newFeatureIds);
        foreach ($removedFeatureIds as $featureId) {
            $feature = Feature::find($featureId);
            $feature?->decrementUsage();
        }

        // Increment usage for new features
        $addedFeatureIds = array_diff($newFeatureIds, $oldFeatureIds);
        foreach ($addedFeatureIds as $featureId) {
            $feature = Feature::find($featureId);
