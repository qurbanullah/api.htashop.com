...
class PackageSortAction
{
     ...
     */
    public function handle($modelId, $newPosition): Package
    {
        $model = (new PackageSearchByIdAction)->handle($modelId);
        $currentPostion = $model->sorting;

        $modelNeedToBeShifted = Package::whereBetween(
            'sorting',
            [
