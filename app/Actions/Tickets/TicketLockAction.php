 ...
 * @link     https://github.com/qurbanullah
 */
class TicketLockAction
{
    /**
     * Lock or unlock a ticket record in database.
     *
     * @param  int  $id  model id
     * @param  bool  $isLocked  whether to lock or unlock the ticket
     * @throws \Illuminate\Auth\Access\AuthorizationException
     */
    public function handle(int $id, bool $isLocked = true): Ticket
    {
        return DB::transaction(
            function () use ($id, $isLocked) {
                $ticket = Ticket::findOrFail($id);

                // Check if user can lock/unlock this ticket
