...
use App\Mail\Tickets\TicketArchived;
...
class SendTicketArchivedEmail implements ShouldQueue
{
        ...
        try {
            ...
            if ($this->ticket->submitter_email) {
                Mail::to($this->ticket->submitter_email)
                    ->send(new TicketArchived($this->ticket, 'creator'));
            }
            ...
            if ($assignment && $assignment->assignedTo && $assignment->assignedTo->email) {
                Mail::to($assignment->assignedTo->email)
                    ->send(new TicketArchived($this->ticket, 'assignee'));
            }
        } catch (\Exception $e) {
            Log::error('Failed to send ticket archived emails', [
    ...
    {
        Log::error('SendTicketArchivedEmail job failed after all retries', [
            'ticket_id' => $this->ticket->id,
