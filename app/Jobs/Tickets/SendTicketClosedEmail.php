...
class SendTicketClosedEmail implements ShouldQueue
{
        ...
        try {
            ...
            if ($this->ticket->submitter_email) {
                Mail::to($this->ticket->submitter_email)
                    ->send(new TicketClosed($this->ticket, 'creator'));
            }
            ...
            if ($assignment && $assignment->assignedTo && $assignment->assignedTo->email) {
                Mail::to($assignment->assignedTo->email)
                    ->send(new TicketClosed($this->ticket, 'assignee'));
            }
        } catch (\Exception $e) {
            Log::error('Failed to send ticket closed emails', [
    ...
    {
        Log::error('SendTicketClosedEmail job failed after all retries', [
            'ticket_id' => $this->ticket->id,
